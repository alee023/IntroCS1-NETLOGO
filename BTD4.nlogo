globals [ health end? balloonsCreated money lvlMoney numBalloons numDead
          addLevelMoney? originalBloonHealth numCreated lvlEvery colorTrack ]
...
to balloonMotion
  ...
  if Levels = "level 10" [ set lvlEvery 0.01 ]
  createBalloons
  ask balloons
    [ fd 0.1
...
to newGame
  ...
  set-default-shape dart-monkeys "monkey"
...
to buy-dart-monkey
  if money - 100 >= 0
    [ if mouse-inside? and [ pcolor ] of patch mouse-xcor mouse-ycor != colorTrack
      [ set money ( money - 100 )
        create-dart-monkeys 1
...
to dart-monkey-upgrade
 ask min-one-of dart-monkeys [distancexy mouse-xcor mouse-ycor]
   [set dart-defense-level dart-defense-level + 1
        ...
        if dart-defense-level = 2 [set money money - (100 / 4) set moneySpent moneySpent + 100 / 4]
        if dart-defense-level = 3 [set money money - ((100 / 4) + 25) set moneySpent moneySpent + (100 / 4) + 25]
        if dart-defense-level = 4 [set money money - ((100 / 4) + 50) set moneySpent moneySpent + (100 / 4) + 50]
        if dart-defense-level = 5 [set money money - 1000 set moneySpent moneySpent + 1000]
        if dart-defense-level = 6 [ set dart-defense-level 5 ]]
end
...
to buy-tack-shooter
  if money - 300 >= 0
    [ if mouse-inside? and [ pcolor ] of patch mouse-xcor mouse-ycor != colorTrack
      [ set money ( money - 300 )
        create-tack-shooters 1
          [ set color 15 set size 1 setxy mouse-xcor mouse-ycor
            set tack-shooter-defense-level 1 set moneyspent moneyspent + 300 ]]]
end
...
to tack-shooter-upgrade
  ...
  if tack-shooter-defense-level = 2 [set money money - (300 / 5) set moneySpent moneySpent + 60]
  if tack-shooter-defense-level = 3 [set money money - ((300 / 5) + 30) set moneySpent moneySpent + 90]
  if tack-shooter-defense-level = 4 [set money money - ((300 / 5) + 60) set moneySpent moneySpent + 120]
  if tack-shooter-defense-level = 5 [set money money -  500 set moneySpent moneySpent + 500]
  if tack-shooter-defense-level = 6 [ set tack-shooter-defense-level 5 ]]
end
...
to buy-bomb-tower
  if money - 300 >= 0 [
  if mouse-inside? and [ pcolor ] of patch mouse-xcor mouse-ycor != colorTrack
    [ create-bomb-towers 1
      [ set color 5 set size 1 setxy mouse-xcor mouse-ycor set bomb-tower-defense-level 1
...
to bomb-tower-upgrade
  ask min-one-of bomb-towers [distancexy mouse-xcor mouse-ycor]
   [set bomb-tower-defense-level bomb-tower-defense-level + 1
       ...
       if bomb-tower-defense-level = 2 [set money money - (300 / 5) set moneySpent moneySpent + 60]
       if bomb-tower-defense-level = 3 [set money money - ((300 / 5)  + 30) set moneySpent moneySpent + 90]
       if bomb-tower-defense-level = 4 [set money money - ((300 / 5)  + 60) set moneySpent moneySpent + 120]
       if bomb-tower-defense-level = 5 [set money money - 800 set moneySpent moneySpent + 800]
       if bomb-tower-defense-level = 6 [ set bomb-tower-defense-level 5 ]]
end
  ...
  if money - 600 >= 0 [
    if mouse-inside? [create-monkey-aces 1
      [ set color 55 set size 2 setxy mouse-xcor mouse-ycor set monkey-ace-defense-level 1
        set heading 315 set moneySpent moneySpent + 600]]
    set money money - 600 ]
end
...
to monkey-ace-upgrade
 ask min-one-of monkey-aces [distancexy mouse-xcor mouse-ycor]
  [set monkey-ace-defense-level monkey-ace-defense-level + 1
     ...
     if monkey-ace-defense-level = 2 [set money money - 120 set moneySpent moneySpent + 120]
      if monkey-ace-defense-level = 3 [set money money - 170 set moneySpent moneySpent + 170]
      if monkey-ace-defense-level = 4 [set money money - 220 set moneySpent moneySpent + 220]
      if monkey-ace-defense-level = 5 [set money money - 1800 set moneySpent moneySpent + 1800]
      if monkey-ace-defense-level = 6 [ set monkey-ace-defense-level 5 ]
...
to buy-super-monkey
  if money - 1000 >= 0
    [ if mouse-inside? and [ pcolor ] of patch mouse-xcor mouse-ycor != colorTrack
      [ create-super-monkeys 1 [ set color 15 set size 1 setxy mouse-xcor mouse-ycor
        set super-monkey-defense-level 1 set moneySpent moneySpent + 1000 ]]
...
to super-monkey-upgrade
  ...
  [  set super-monkey-defense-level super-monkey-defense-level + 1
     if super-monkey-defense-level = 2 [set money money - 200 set moneySpent moneySpent + 200]
     if super-monkey-defense-level = 3 [set money money - 300 set moneySpent moneySpent + 300]
     if super-monkey-defense-level = 4 [set money money - 400 set moneySpent moneySpent + 400]
     if super-monkey-defense-level = 5 [set money money - 3000 set moneySpent moneySpent + 3000]
     if super-monkey-defense-level = 6 [ set super-monkey-defense-level 5 ]
  ]
end
...
301
Dart Monkey Upgrade
dart-monkey-upgrade\nask dart-monkeys [ set label dart-defense-level ]
NIL
1
T
OBSERVER
...
tack-shooter-upgrade\nask tack-shooters [ set label tack-shooter-defense-level ]
NIL
1
T
OBSERVER
...
451
Bomb Tower Upgrade
bomb-tower-upgrade\nask bomb-towers [ set label bomb-tower-defense-level ]
NIL
1
T
OBSERVER
...
81
Monkey Ace Upgrade
monkey-ace-upgrade\nask monkey-aces [ set label monkey-ace-defense-level ]
NIL
1
T
OBSERVER
...
super-monkey-upgrade\nask super-monkeys [ set label super-monkey-defense-level ]
NIL
1
T
OBSERVER
...
Each time a balloon reaches the end of the track, the player loses 1 life. Each time a balloon is "popped" by a defense, the player gains 1 life. The player will receive money at the end of each round.

## User controls / How to play

### How to Start
* Choose a map from maps chooser, with Map 1 being the easiest map and Map 3 being the hardest. Afterwards, pick a level. Then click the 'New Game' button AND the 'New Level' button.
### Going to the Next Level
* If you want to play a new level and not have the money reset, click the 'New Level' button and click 'Go' when you're ready. NOTE that 'New Level' only works if you are finished with one already (because no cheating, okay).
### How to Buy & Upgrade
* To set down a defense, have your mouse over the desired area and press the appropriate action key. To upgrade a certain defense, place your mouse over the defense and press the action key for the upgrade.
Be sure to click the white space outside the map first. Otherwise, you'd just be typing in numbers in the command center.
...
* Upgrading improves damage and range
* Unlock massive range at level 5
...
### Added Features
#### Alison
- Continued to work on levels and things related to the number of balloons created, etc. in each level.
...
true
0
Circle -6459832 true false 90 45 118
Circle -1184463 true false 103 58 92
Circle -16777216 true false 112 88 24
Circle -16777216 true false 163 89 22
Rectangle -2674135 true false 119 126 179 130
Circle -6459832 true false 140 103 18
Circle -6459832 true false 174 26 52
Circle -6459832 true false 74 26 50
Rectangle -6459832 true false 117 152 180 256
Rectangle -6459832 true false 174 154 268 171
Rectangle -6459832 true false 20 154 126 170
Rectangle -6459832 true false 117 226 142 308
Rectangle -1184463 true false 126 166 174 222
Rectangle -6459832 true false 155 224 180 306
